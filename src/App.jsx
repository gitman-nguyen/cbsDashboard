        ...
        <div ref={ref} className="bg-white text-gray-800 p-8 font-sans">
            ...
            <section className="mb-12 grid grid-cols-1 lg:grid-cols-5 gap-6 page-break-avoider">
                ...
                <div className="lg:col-span-3 border border-gray-200 p-4 rounded-lg shadow-sm">
                    ...
                    <ResponsiveContainer width="100%" height={300}>
                        <LineChart data={avgDailyTransactionOverviewData} isAnimationActive={false} margin={{ top: 10, right: 30, left: 20, bottom: 0 }}>
                            <CartesianGrid strokeDasharray="3 3" />
                            <XAxis dataKey="name" tick={{ fontSize: 10 }} />
                            <YAxis tickFormatter={(value) => `${(value / 1000000).toFixed(1)}M`} tick={{ fontSize: 10 }} />
                            <Tooltip />
                            <Legend />
                            <Line type="monotone" dataKey="NgayThuong" name="Ngày thường" stroke="#8884d8" dot={false} />
                            <Line type="monotone" dataKey="CuoiTuan" name="Cuối tuần" stroke="#82ca9d" dot={false} />
                        </LineChart>
                    </ResponsiveContainer>
                </div>
            </section>
        ...
        <MainContainer>
            ...
            <main id="dashboard-content" className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
                ...
                <section className="mb-16 grid grid-cols-1 lg:grid-cols-5 gap-6 page-break-avoider">
                    ...
                    <div className="lg:col-span-3"><ChartCard title="Số lượng giao dịch trung bình theo ngày"><ResponsiveContainer width="100%" height="100%"><LineChart data={historicalData.avgDailyTransactionOverviewData} isAnimationActive={true} margin={{ top: 10, right: 30, left: 20, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" stroke="rgba(255, 255, 255, 0.1)" /><XAxis dataKey="name" stroke="rgba(255, 255, 255, 0.5)" tick={{ fontSize: 12 }} /><YAxis stroke="rgba(255, 255, 255, 0.5)" tickFormatter={(value) => `${(value / 1000000)}M`} tick={{ fontSize: 12 }} /><Tooltip content={<CustomTooltip valueFormatter={val => val.toLocaleString()}/>} /><Legend wrapperStyle={{color: "#d1d5db"}} /><Line type="monotone" dataKey="NgayThuong" name="Ngày thường" stroke="#8884d8" strokeWidth={2} dot={false} /><Line type="monotone" dataKey="CuoiTuan" name="Cuối tuần" stroke="#82ca9d" strokeWidth={2} dot={false} /></LineChart></ResponsiveContainer></ChartCard></div>
                </section>
